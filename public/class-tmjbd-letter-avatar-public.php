 ...
 */
class TMJBD_Letter_Avatar_Public{
	...
	public function enqueue_styles() {
		 ...
		 * defined in TMJBD_Letter_Avatar_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The TMJBD_Letter_Avatar_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

		// if ( is_admin_bar_showing() && TMJBD_letter_avatar()->is_active() ) {
		// 	wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/TMJBD-letter-avatar-public.css', array(), $this->version, 'all' );
		// }
	}
	...
	public function enqueue_scripts() {
		...
		//wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/TMJBD-letter-avatar-public.js', array( 'jquery' ), $this->version, false );
	...
	public function get_avatar_url( $url, $id_or_email, $args ) {
		...
		if ( is_admin() && function_exists( 'get_current_screen' ) && $screen = get_current_screen() ) {
			...
			if ( in_array( $screen, array( 'dashboard', 'edit-comments' ) ) ) {
				if ( tmjbd_letter_avatar()->is_active() ) {
					$args['default'] = 'tmjbd_letter_avatar';
				}
			}
		}
		...
		//$args = apply_filters( 'tmjbd_letter_avatar_args', $args, $id_or_email );
		...
		if ( $force_default && isset( $args['default'] ) && $args['default'] !== 'tmjbd_letter_avatar' ) {
			/**
		...
		if ( ! isset( $args['default'] ) || $args['default'] !== 'tmjbd_letter_avatar' ) {
			/**
	...
	public function generate_letter_avatar_url( $id_or_email, $args = array() ) {
		 ...
		 */
		$current_option = get_network_option( null, 'tmjbd_letter_avatar_method', 'auto' );
		...
		switch ( $current_option ) {
			case  'fixed':
				$bg = get_network_option( null, 'tmjbd_letter_avatar_bg' );
			...
			case 'random':
				...
				if ( $user instanceof WP_User ) {
					$bg = $user->get( '_tmjbd_letter_avatar_bg' );
				} else if ( $id_or_email instanceof WP_Comment ) {
					$bg = get_comment_meta( $id_or_email->comment_ID, '_tmjbd_letter_avatar_bg' );
				}

				$backgrounds = get_network_option( null, 'tmjbd_letter_avatar_bgs', 'fc91ad' );
				...
				if ( empty( $bg ) || ! in_array( $bg, $backgrounds ) || ! ctype_xdigit( $bg ) ) {
					...
					if ( $user instanceof WP_User ) {
						update_user_meta( $user->ID, '_tmjbd_letter_avatar_bg', $bg );
					} else if ( $id_or_email instanceof WP_Comment ) {
						update_comment_meta( $id_or_email->comment_ID, '_tmjbd_letter_avatar_bg', $bg );
					}
				}
				break;
			case 'auto':
		 ...
		 */
		$color_method = get_network_option( null, 'tmjbd_letter_avatar_color_method', 'auto' );
		...
		if ( $color_method == 'fixed' ) {
			$color = get_network_option( null, 'tmjbd_letter_avatar_color', 'ffffff' );
		}
		...
		$letters_count = get_network_option( null, 'tmjbd_letter_avatar_letters', 2 );
		...
		if ( get_network_option( null, 'tmjbd_letter_avatar_uppercase', true ) ) {
			/**
		...
		$url_args = array(
			...
			'name'       => $letters,
			'rounded'    => get_network_option( null, 'tmjbd_letter_avatar_rounded', true ),
			'background' => $bg,
			'bold'       => get_network_option( null, 'tmjbd_letter_avatar_bold', false ),
			//'uppercase'  => '', //already set
			'color'      => $color,
			'format'     => get_network_option( null, 'tmjbd_letter_avatar_format', 'svg' )
		);

		$url_args = apply_filters( 'tmjbd_letter_avatar_url_args', $url_args, $id_or_email );
	...
	public function generate_image( $data ) {
		...
		$format = in_array( $format, tmjbd_letter_avatar()->sanitizer->get_formats() ) ? $format : 'svg';
		if ( $format != 'svg' ) {
		...
		if ( $save_to_uploads ) {
			...
			//Avatar folder
			$avatar_folder = apply_filters( 'tmjbd_letter_avatar_upload_folder', 'letter-avatar' );
	...
	protected function image_content( $data ) {
		...
		if ( $format != 'svg' ) {
			 ...
			 * @since 1.3.4
			 */
			$font = apply_filters( 'tmjbd_letter_avatar_image_font', $font, $data );
		 ...
		 * @since 1.3.3
		 */
		$avatar = apply_filters( 'tmjbd_letter_avatar_image_content', $avatar, $data );
	...
	public function gravatar_exist( $email ) {
		...
		$cache = wp_cache_get( $hash, 'tmjbd_letter_avatar_gravatar', false, $found );
		if ( $found ) {
			return $cache;
		}
		...
		wp_cache_set( $hash, $has_valid_avatar, 'tmjbd_letter_avatar_gravatar', $expire );
