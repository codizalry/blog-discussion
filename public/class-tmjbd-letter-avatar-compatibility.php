 ...
 * @since      1.0.0
 */
class TMJBD_Letter_Avatar_Compatibility{
	...
	public function bp_core_fetch_avatar_no_grav( $no_gravatar, $params ) {
		...
		if ( $force_default && $default !== 'tmjbd_letter_avatar' ) {
			return $no_gravatar;
		}
		 ...
		 */
		if ( $default == 'tmjbd_letter_avatar' || tmjbd_letter_avatar()->is_active() ) {
		...
		if ( $default == 'tmjbd_letter_avatar' || tmjbd_letter_avatar()->is_active() ) {
			...
			if ( $object == 'user' ) {
				if ( isset( $params['item_id'] ) ) {
					...
					$url  = tmjbd_letter_avatar()->public->generate_letter_avatar_url( $user, $args );
					if ( $url ) {
						/**
	...
	public function um_user_avatar_url_filter( $url, $user_id, $data = array( 'size' => 96 ) ) {
		...
		if ( ! um_profile( 'profile_photo' ) && ! um_user( 'synced_profile_photo' ) && ! UM()->options()->get( 'use_gravatars' ) ) {
			...
			$avatar = tmjbd_letter_avatar()->public->generate_letter_avatar_url( $user_id, $args );
			if ( $avatar ) {
				$url = $avatar;
			}
		}
	...
	public function wpdiscuz_get_avatar_url( $url, $id_or_email, $args ) {
		...
		if ( isset( $args['wpdiscuz_current_user'] ) ) {
			...
			$avatar = tmjbd_letter_avatar()->public->get_avatar_url( $url, $current_user, $args );
			if ( $avatar ) {
				$url = $avatar;
			}
		}
