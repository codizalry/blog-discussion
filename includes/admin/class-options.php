...
class Options {
	...
	public static function get_options( $force = false, $key = '' ) {
		...
		if ( ! is_array( $options ) || empty( $options ) || true === $force ) {
			$options       = get_site_option( 'tmjbd_options', array() );
	...
	public static function update_options( $options = array() ) {
		...
		if ( \Simple_Comment_Editing::is_multisite() ) {
			update_site_option( 'tmjbd_options', $options );
		} else {
			update_option( 'tmjbd_options', $options );
		}
	}
	...
	private static function get_defaults() {
		 ...
		 */
		$defaults = apply_filters( 'tmjbd_options_defaults', $defaults );
		return $defaults;
	}
}
